;(function () {
    ...
    Lampa.Lang.add({
        not_supported_track: {
            ru: 'Ð½Ðµ Ð¿Ð¾Ð´Ð´ÐµÑ€Ð¶Ð¸Ð²Ð°ÐµÑ‚ÑÑ',
            uk: 'Ð½Ðµ Ð¿Ñ–Ð´Ñ‚Ñ€Ð¸Ð¼ÑƒÑ”Ñ‚ÑŒÑÑ',
            en: 'not supported',
            be: 'Ð½Ðµ Ð¿Ð°Ð´Ñ‚Ñ€Ñ‹Ð¼Ð»Ñ–Ð²Ð°ÐµÑ†Ñ†Ð°',
        },
    })
    ...
    function subscribe(data) {
        ...
        function setSubs() {
            if (inited_parse) {
                ...
                var minus =
                    inited_parse.streams.filter(function (a) {
                        return a.codec_type == 'audio'
                    }).length + 1
            ...
            if (inited_parse) {
                ...
                if (parse_tracks.length !== video_tracks.length) {
                    if (webOS.sdk_version < 5) {
                        ...
                        non_supported = inited_parse.streams.filter(function (
                            a
                        ) {
                            return a.codec_name == 'truehd'
                        })
                ...
                if (webOS.sdk_version > 4) {
                    for (var r = 0; r < parse_tracks.length; r++) {
                        if (parse_tracks[r].disposition) {
                            if (parse_tracks[r].disposition.default === 1)
                                parse_tracks.splice(
                                    0,
                                    0,
                                    parse_tracks.splice(r, 1)[0]
                                )
                        }
                    }
                }
                ...
                parse_tracks.forEach(function (track, i) {
                    if (video_tracks[i]) {
                        ...
                        video_tracks[i].label =
                            (track.tags.title ||
                                track.tags.handler_name ||
                                Lampa.Lang.translate('player_unknown')) +
                            ' / ' +
                            Math.round(
                                (track.bit_rate ||
                                    track.tags.BPS ||
                                    track.tags['BPS-eng']) / 1000
                            ) +
                            ' kbps'
                    }
                })
                    ...
                    var elem = {
                        ...
                        extra: {
                            ...
                            fourCC:
                                track.codec_name +
                                ' - ' +
                                Lampa.Lang.translate('not_supported_track'),
                        },
                        ghost: true,
                    }
            ...
            if (inited_parse) {
                ...
                parse_subs.forEach(function (track, a) {
                    ...
                    if (video_subs[i]) {
                        ...
                        video_subs[i].label =
                            track.tags.title || track.tags.handler_name
                    }
                })
            }
        }
